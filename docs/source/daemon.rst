...
The module offers an interface to interact with uPlexa daemon. For the time being, like with
wallet, the only available backend is JSON RPC.

.. code-block:: python

    In [1]: from uplexa.daemon import Daemon

    In [2]: from uplexa.backends.jsonrpc import JSONRPCDaemon
...
.. code-block:: python
    ...
    In [5]: from uplexa.wallet import Wallet

    In [6]: from uplexa.backends.jsonrpc import JSONRPCWallet
...
.. code-block:: python
    ...
    ~/devel/uplexa-python/uplexa/daemon.py in send_transaction(self, tx, relay)
         10 
    ...
    ~/devel/uplexa-python/uplexa/backends/jsonrpc.py in send_transaction(self, blob, relay)
