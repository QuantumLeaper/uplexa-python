...
Payments in uPlexa desire a bit of explanation even for people experienced with cryptocurrency.

The main difference from coins which use transparent blockchain is that uPlexa transactions do not
disclose sender or recipient's address, nor they tell what the amount is. This is a great feature
that makes uPlexa stand out, however at the same time it causes difficulties. In the outgoing
...
With uPlexa releases > 0.11.x you will be also able to filter payments by the address:
...
.. warning:: The presence of a transaction in the mempool is an indication that someone has already
    ...
    **With significant amounts you should also wait for a few confirmations to appear.** The top
    ...
    wait for at least 6 confirmations to appear, which is also the standard in uPlexa before funds
...
There are two methods for sending uPlexa. For a single payment use the ``transfer`` method of
