# uPlexaPy - A python toolbox for uPlexa
# Copyright (C) 2016 The uPlexaPy Developers.
#
# uPlexaPy is released under the BSD 3-Clause license. Use and redistribution of
...
def encode(hex):
    '''Encode hexadecimal string as base58 (ex: encoding a uPlexa address).'''
    data = _hexToBin(hex)
    l_data = len(data)
...
def decode(enc):
    '''Decode a base58 string (ex: a uPlexa address) into hexidecimal form.'''
