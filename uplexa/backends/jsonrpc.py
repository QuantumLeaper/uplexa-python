...
class JSONRPCDaemon(object):
    """
    JSON RPC backend for uPlexa daemon
    ...
    """
    def __init__(self, protocol='http', host='127.0.0.1', port=21061, path='/json_rpc', user='', password=''):
...
class JSONRPCWallet(object):
    """
    JSON RPC backend for uPlexa wallet (``uplexa-wallet-rpc``)
    ...
    def accounts(self):
        ...
        try:
            ...
            # uplexa <= 0.11 : there's only one account and one address
            _log.debug('uPlexa <= 0.11 found, no accounts')
    ...
    def addresses(self, account=0):
        ...
        if 'addresses' not in _addresses:
            # uplexa <= 0.11
            _log.debug('uPlexa <= 0.11 found, assuming single address')
...
_err2exc = {
    ...
    -43: exceptions.WalletIsNotDeterministic, # https://github.com/uplexa/uplexa/pull/4653
